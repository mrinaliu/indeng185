...
my_queue = Queue('stemfem', Exchange('stemfem'), 'routing_key')

BROKER_URL='amqp://guest:guest@rabbitmq/stemfem-vhost'
...
# converting messages received to celery tasks
    def handle_message(self, body, message):
        ...
        with Connection(BROKER_URL) as conn:
            ...
            if messagedict.get("user") == "mentor":
                args = [messagedict.get("name"), messagedict.get("profession"), messagedict.get("availability")]
            elif messagedict.get("user") == "mentee":
                args = [messagedict.get("name"), messagedict.get("grade"), messagedict.get("availability")]
            ...
            if not exit:
                createTask = {
                    "id": str(messagedict.get("name")),
                    "args": args,
                    "task": f'task.{messagedict.get("user")}'
                             }
